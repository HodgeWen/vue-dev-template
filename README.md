...
### 其他
[css规范指南(酌情参考)](https://en.bem.info/methodology/quick-start/)
...
> operations 是一个二维数组或者一个返回二维数组的函数, 其中第二维的数组的第一个元素是按钮名称, 第二个元素是注册到表格的事件, 如果是一个函数， 则函数的参数返回的是一个表格记录的对象. 这是一个典型的装饰器模式, 我们可以动态地向组件注册事件, 通常来讲, 这个选项可以满足9成以上的操作按钮需求, 如果按钮过多(建议将按钮分开放到对应的列当中)或者有特殊的需求请使用render选项(比如一个开关操作, 尽管button也能解决问题, 但更多的会使用Switch组件).
